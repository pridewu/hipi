...
function T($template='',$layer=''){
    ...
    //===============update fyw 2014-3-18======================
    $fileName = $baseUrl.($theme?$theme.'/':'').$file.C('TMPL_TEMPLATE_SUFFIX');
    if(!is_file($fileName) && C('PARENT_MODULE')){
    	$fileName = APP_PATH.C('PARENT_MODULE').'/'.$layer.'/'.($theme?$theme.'/':'').$file.C('TMPL_TEMPLATE_SUFFIX');
    }
    return $fileName;
...
function R($url,$vars=array(),$layer='') {
    ...
    if($class){
        ...
        if(!method_exists($class,$action.C('ACTION_SUFFIX')))  return false; //update fyw 2014-3-17;
