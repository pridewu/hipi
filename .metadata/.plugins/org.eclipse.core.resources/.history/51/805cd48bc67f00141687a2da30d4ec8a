...
function add(){
	addData("添加","#edit_form",datagrid,"/System/Upfile/add",edit_W,edit_H);
}
function edit(rowIndex,rowData){
	editData(rowIndex,rowData,"编辑",'#edit_form',datagrid,"/System/Upfile/edit",edit_W,edit_H);
}
function del(){
	delData(datagrid,"/System/Upfile/del");
}
...
function del(rowIndex,rowData){
    edit_W = 350;
    edit_H = 200;
	editData(rowIndex,rowData,"删除数据",'#edit_form',datagrid,"/System/Upfile/del",edit_W,edit_H);
}
function count(){
	post("/System/Upfile/count",true,null,$.serializeObject($('#search_form')));
}
