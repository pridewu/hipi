...
<div data-options="region:'center',split:true,border:false" style="overflow: hidden;">
	...
	角色：<input type="text" name="where[id]" placeholder="请输入角色名称或ID" style="width: 160px"></input>	
...
$(function(){
	...
	columns:[[  
	    ...
	    {field:'name',title:'角色名',sortable:true,width:200},
        {field:'isSuper',title:'超级管理',sortable:true,width:100,formatter: function(value,row,index){return value==1 ? '<font color=red>是</font>' : '否';}},
...
function add(){
	addData("添加","#edit_form",datagrid,"/System/SysRole/add",edit_W,edit_H);
}
function edit(rowIndex,rowData){
	editData(rowIndex,rowData,"编辑",'#edit_form',datagrid,"/System/SysRole/edit",edit_W,edit_H);
}
function del(){
	delData(datagrid,"/System/SysRole/del");
}
